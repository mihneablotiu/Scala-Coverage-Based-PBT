...
object Tactic {
  ...
  /** Perturb a corpus seed — an input that previously grew coverage (FuzzChick / AFL). Bias hard toward the *most recent* grower: it sits on the
    * current coverage frontier, so mutating it is the springboard that ratchets one rung deeper (extend the sorted prefix, grow the tree). A slice of
    * draws still pick an older seed for diversity. This lightweight power schedule is what lets mutation climb deep structured targets instead of
    * spinning on shallow seeds.
    */
