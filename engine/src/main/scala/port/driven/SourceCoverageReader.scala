...
/** Reads source-level coverage of one method in one source file.
  ...
  * compiler plugin so every source-level branch is instrumented at compile time. Cheap enough to
  * call per fuzz iteration — the static map is cached, the measurement files are small text logs.
  */
