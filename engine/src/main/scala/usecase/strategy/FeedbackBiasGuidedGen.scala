...
/** **Placeholder** for a feedback-aware custom-`Gen` strategy.
  ...
  * new coverage, or biasing element distributions toward values seen in inputs that hit new arms.
  * The `Gen.delay` wrapper ensures the closure re-evaluates each iteration so the latest `feedback`
  * state is in scope.
  *
