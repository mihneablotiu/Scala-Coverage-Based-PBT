...
/** **Placeholder** for a corpus + mutation guided strategy (AFL-style).
  ...
  * (visible via `feedback.history`'s `newlyCoveredBranches`) and produce next inputs by sampling
  * a corpus entry and applying a type-specific mutation. The `Gen.delay` wrapper ensures the
  * closure re-evaluates each iteration so the latest `feedback` state is in scope.
  *
